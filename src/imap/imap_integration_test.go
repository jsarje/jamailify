...
func TestImapIntegration(t *testing.T) {
	...
	imapPort, err := greenmailContainer.MappedPort(ctx, "3143/tcp")
	require.NoError(t, err)
	...
	if len(uids) == 2 {
		...
		for _, uid := range uids {
			retrievedEmail, err := imapClient.DownloadEmail(uid)
			require.NoErrorf(t, err, "failed to FETCH body for uid=%s", uid)
...
func sendMail(t *testing.T, addr, to, subject, body string) error {
	...
	// the retrieval path under test is IMAP authentication rather than SMTP auth.
